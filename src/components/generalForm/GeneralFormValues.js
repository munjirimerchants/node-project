import { GENERAL_ENQUIRY_FIELDS as FIELD_NAMES } from "../../forms/FormFieldNames";
import UserDetails from "../../utils/UserDetails";
import { useEffect, useState } from "react";
import * as Yup from "yup";

const GeneralFormValues = (initialEnquiryState) => {
  const { loading, userDetails } = UserDetails();
  const [userDetailsState, setUserDetailsState] = useState();
  const [initialEnquiryType, setInitialEnquiryType] = useState("machine");

  useEffect(() => {
    const dynamicValue = initialEnquiryState;
    setInitialEnquiryType(dynamicValue);
  }, []);
  ...
  const initialValues = {
    [FIELD_NAMES.enquiryType]: initialEnquiryType,
...
export default GeneralFormValues;
